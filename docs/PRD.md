...
| **T002** | web_fetch | URL로 HTTP 요청 | High | 예 | 웹 페이지 콘텐츠 가져오기 | ⚠️ 부분 구현 (GET만 지원, POST/PUT/DELETE는 향후 지원 예정) |
| **T003** | browser | Playwright 브라우저 자동화 | Critical | 예 | 동적 웹사이트 스크래핑 | ❌ 미구현 |
| **T004** | file_read | 파일 읽기 | High | 예 | 로컬 파일 내용 확인 | ✅ 완료 |
| **T005** | file_write | 파일 쓰기 | Critical | 예 | 설정 파일 수정 | ✅ 완료 |
| **T006** | channel_send | 메신저로 메시지 전송 | Medium | 예 | 다른 채널로 알림 전송 | ❌ 미구현 |
...
> **참고**: 현재 GET 메서드만 지원되며, POST/PUT/DELETE는 향후 지원 예정입니다.
...
| 제한 항목 | 값 | 설명 | 구현 상태 |
|-----------|-----|------|-----------|
| 최대 반복 횟수 | 10회 | 무한 루프 방지 | ✅ 구현됨 |
| 단일 도구 타임아웃 | 30초 | exec, web_fetch | ✅ 구현됨 |
| 브라우저 타임아웃 | 60초 | browser 도구 | ✅ 구현됨 |
| 총 실행 시간 제한 | 5분 | 전체 흐름 제한 | ✅ 구현됨 |
| 출력 크기 제한 | 1MB | 과도한 데이터 방지 | ✅ 구현됨 (모든 도구에 적용) |
...
- [x] 최대 반복 횟수 제한 (10회)
- [x] 총 실행 시간 제한 (5분)
- [x] 출력 크기 제한 (1MB) - 모든 도구에 적용
- [ ] 위험도 기반 승인 시스템 (requestApproval)
- [ ] 구조화된 도구 실행 로깅 (ToolExecutionLog)
- [ ] browser 도구 구현
- [ ] channel_send 도구 구현
- [ ] web_fetch POST/PUT/DELETE 지원
